...
/////////////// phone 分機表
       ...
       {
        title: 'phones',       templateUrl: 'partials/phones.html'   , controller: 'phoneCtrl'
       })
      .when('/edit-phone/:phoneID', 
      {
        title: 'Edit Phone',     templateUrl: 'partials/edit-phone.html'       ,
        controller: 'editPhoneCtrl' ,
         resolve: {
    editphone: function(myservices, $route){ // route 需等待 promise fulfilled 完成 才將myphones 注入editPhoneCtrl()
            var phoneID = $route.current.params.phoneID;
            return myservices.getNCUT_Phone(phoneID); /// editPhoneCtrl( .... myphones)   // route 會將 return 存放於 myphones.data  接收 這個return
                                           }
                  }
...
myApp.run(function ($rootScope, $location, $http, $timeout, AuthService, RESTService) {
    ...
    // *****
    //    登入 Initialize authentication
    // *****
    $rootScope.authService = AuthService;

    // text input for login/password (only)
    $rootScope.loginInput = 'admin';
    $rootScope.passwordInput = 'admin';
    ...
    $rootScope.$watch('authService.authorized()', function () {
        ...
        if ($rootScope.authService.authorized()) {
            $location.path("/");
            $rootScope.app_adminauth = true  ;  // 開啟權限
        ...
        if (!$rootScope.authService.authorized()) {
            $location.path("/");
            $rootScope.app_adminauth = false   ; // 關閉權限
